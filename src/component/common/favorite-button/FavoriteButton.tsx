...
const FavoriteButton = (props: FavoriteButtonProps) => {
    ...
    const getTooltipText = (isFavorite: boolean, itemType: string) => {
        if (isFavorite) {
            return itemType === 'theater' ? 'Retirer des favoris' : 'Retirer de la watchlist';
        } else {
            return itemType === 'theater' ? 'Ajouter aux favoris' : 'Ajouter à la watchlist';
        }
    };
    const tooltipText = getTooltipText(isFavorite, itemType);

    const buttonText = `Ce ${itemType === 'theater' ? 'cinéma' : 'film'} ${isFavorite ? 'est' : 'n’est pas'} dans ${itemType === 'theater' ? 'vos favoris' : 'votre watchlist'}`;
    ...
    const addToFavorites = async () => {
        ...
        try {
            if (itemType === 'movie') {
                await addToUserFavMovies(currentUser.id, itemId);
                refreshFavoriteMovies();
                setFavoriteValue(favoriteMovies);
                toast.success('Ajouté à la watchlist');
            } else if (itemType === 'theater') {
                await addToUserFavTheaters(currentUser.id, itemId);
    ...
    const removeFromFavorites = async () => {
        ...
        try {
            if (itemType === 'movie') {
                await deleteUserFavMovie(currentUser.id, itemId);
                refreshFavoriteMovies();
                setFavoriteValue(favoriteMovies);
                toast.success('Retiré de la watchlist');
            } else if (itemType === 'theater') {
                await deleteUserFavTheater(currentUser.id, itemId);
                refreshFavoriteTheaters();
                setFavoriteValue(favoriteTheaters);
    ...
    return (
        <>
            ...
            >
                ...
                <span className={style.favoriteButtonText}>{buttonText}</span>
            </button>
        </>
    );
};
